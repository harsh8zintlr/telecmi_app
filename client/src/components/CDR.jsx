...
const CDR = () => {
  ...
  const userEndpoints = [
    { id: 'user-in', label: 'User Incoming', func: getUserInCDR },
    { id: 'user-out', label: 'User Outgoing', func: getUserOutCDR },
    { id: 'user-missed', label: 'User Missed', func: getUserMissed },
    { id: 'user-out-missed', label: 'User Out Missed', func: getUserOutMissed },
    { id: 'user-answered', label: 'User Answered', func: getUserAnswered },
    { id: 'user-out-answered', label: 'User Out Answered', func: getUserOutAnswered },
  ];

  const adminEndpoints = [
    { id: 'admin-answered', label: 'Admin Answered', func: getAnswered },
    { id: 'admin-missed', label: 'Admin Missed', func: getMissed },
    { id: 'admin-out-answered', label: 'Admin Out Answered', func: getOutAnswered },
    { id: 'admin-out-missed', label: 'Admin Out Missed', func: getOutMissed },
  ];
  ...
  const handleFetch = async (endpoint) => {
    ...
    try {
      ...
      if (endpoint.id.startsWith('user-')) {
        ...
        if (endpoint.id === 'user-missed' || endpoint.id === 'user-out-missed') {
          delete payload.type;
        }
      } else {
        payload = {
          start_date: filters.start_date ? parseInt(filters.start_date) : undefined,
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', flexWrap: 'wrap' }}>
          {userEndpoints.map((endpoint) => (
            <button
              key={endpoint.id}
              className={`btn ${activeTab === endpoint.id ? 'btn-primary' : 'btn-secondary'}`}
        ...
        <button
          ...
          onClick={() => {
            const endpoint = [...userEndpoints, ...adminEndpoints].find((e) => e.id === activeTab);
                  ...
                  <tbody>
                    {cdrData.cdr.map((call, idx) => (
                      <tr key={idx}>
                        ...
                        <td>
                          <span className={`badge ${call.duration > 0 ? 'badge-success' : 'badge-danger'}`}>
                            {call.duration > 0 ? 'Answered' : 'Missed'}
                          </span>
                        </td>
                        <td>{call.rate || '-'}</td>
                        {cdrData.cdr[0].filename && (
                          <td>{call.filename ? <a href={call.filename} target="_blank" rel="noopener noreferrer">Listen</a> : '-'}</td>
                        )}
                        {cdrData.cdr[0].notes && (
                          <td>{call.notes?.length > 0 ? `${call.notes.length} note(s)` : '-'}</td>
                        )}
                      </tr>
                    ))}
                  </tbody>
